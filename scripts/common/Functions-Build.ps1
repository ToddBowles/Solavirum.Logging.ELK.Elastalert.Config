...
$nugetBuildEngine = {
    ...
    if ($sign)
    {
        throw "Signing is not currently supported for the MSBuildOctopackBuildEngine"
    }

    Write-Host "##teamcity[blockOpened name='Packaging']"

    $nuspecFile = Get-ChildItem -Path $srcDirectoryPath -Filter *.nuspec | Single -Description "Searching for nuspec files in [$srcDirectoryPath]"

    . "$rootDirectoryPath\scripts\common\Functions-NuGet.ps1"

    NuGet-Pack $nuspecFile $buildDirectory -Version $versionChangeResult.Informational -createSymbolsPackage:$false
...
function Build-DeployableComponent
{
    ...
    (
        ...
        [string]$subDirectory,
        [string[]]$projects,
        [ValidateSet("msbuild-octopack", "nuget", "connect-plugin", "npm-nuget")]
        ...
        [ValidateSet("none", "nunit-pester")]
        [string]$preDeploymentTestEngineName="none",
        [scriptblock]$preDeploymentTestEngine,
        [ValidateSet("none", "nunit", "npm-webdriverio")]
    ...
    try
    {
        ...
        if ($deploy)
        {
            ...
            if ($environments | Any)
            {
                ...
                foreach ($project in $projects)
                {
                    New-OctopusDeployment -ProjectName $project -Environment "$initialDeploymentEnvironment" -Version $versionChangeResult.Informational -OctopusServerUrl $octopusServerUrl -OctopusApiKey $octopusServerApiKey -Wait
                }
                ...
                else
                {
                    ...
                    if ($environments.Length -eq 2)
                    {
                        ...
                        foreach ($project in $projects)
                        {
                            New-OctopusDeployment -ProjectName $project -Environment "$secondaryDeploymentEnvironment" -Version $versionChangeResult.Informational -OctopusServerUrl $octopusServerUrl -OctopusApiKey $octopusServerApiKey -Wait
                        }
